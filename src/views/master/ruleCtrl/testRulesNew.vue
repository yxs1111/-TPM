...
<template>
  <div class="app-container" style="border-radius:0px;">
    ...
    <el-table  :data="tableData" :span-method="spanMethod" border :cell-style="cellStyle" :header-cell-style="HeadTable" :row-class-name="tableRowClassName"
      ...
      <el-table-column width="300" align="left" prop="ruleContentAfter" label="">
        <template slot-scope="scope">
          <div v-if="scope.row.ruleUnit === '∈'">
            [&nbsp;
            <el-input v-model="scope.row.startRule" style="width:60px;" size="small" @blur="number($event,scope.row,scope.$index)" />%,
            <el-input v-model="scope.row.endRule" style="width:60px;" size="small" @blur="number($event,scope.row,scope.$index)" />%&nbsp;]
          </div>
          <div v-else>
