...
<template>
  <div class="app-container" style="border-radius: 0px">
    ...
    <el-table :data="tableData" :key="tableKey" :span-method="spanMethod" border :cell-style="cellStyle" :header-cell-style="HeadTable" :row-class-name="tableRowClassName" style="width: 100%" :max-height="maxheight">
      ...
      <el-table-column width="300" align="left" prop="ruleContentAfter" label="">
        <template slot-scope="scope">
          <div v-if="scope.row.ruleUnit === '∈'">
            [&nbsp;
            <el-input v-model="scope.row.startRule" :class="scope.row.status ? '' : 'noPass'" style="width: 60px" size="small" @blur="number($event, scope.row, scope.row.startRule, scope.$index)" />%,
...
export default {
  ...
  data() {
    return {
      ...
      rulesAdd: {
        targetYear: [{ required: true, message: '请输入目标年月', trigger: 'blur' }],
  ...
  methods: {
      ...
      for (let index = 0; index < this.tableData.length; index++) {
        ...
        if (element.ruleUnit === '∈') {
          if (element.endRule === '' || element.startRule === '') {
            ...
            let countNumber = Number(element.startRule) - Number(element.endRule)
            if (countNumber >= 0) {
              flag = 0
              element.startRule = ''
              element.status = 0
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const url = this.isEditor ? API.updateMdPriceGear : API.insertMdPriceGear
          url({
            id: this.editorId,
