 ...
 * @LastEditTime: 2023-02-02 11:40:26
-->
<template>
  <div class="MainContent">
    ...
    <el-table :data="tableData" :key="tableKey" :max-height="maxheight" :min-height="800" border @selection-change="handleSelectionChange" :header-cell-style="HeadTable"
      ...
      <el-table-column prop="systemDate" align="center" width="220" label="系统生效时间">
        <template slot-scope="scope">
          <div v-show="scope.row.isEditor">
            <el-date-picker v-model="scope.row.systemDate" type="monthrange" value-format="yyyyMM" :picker-options="pickerOptionsSystemDate(scope.row)" format="yyyyMM"
  ...
  methods: {
    ...
    async exportData() {
      ...
      await API.exportCustomerContract({
        ...
        let timestamp = Date.parse(new Date())
        downloadFile(res, '直供客户合同录入list-' + timestamp + '.xlsx') //自定义Excel文件名
