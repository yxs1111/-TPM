<template>
  <div class="MainContent">
      ...
      <div class="SelectBar">
        ...
        <div class="Selectli">
          ...
          <el-select v-model="filterObj.channelName" clearable filterable placeholder="请选择" @change="getCustomerList">
            <el-option v-for="(item, index) in channelArr" :key="index" :label="item.channelEsName" :value="item.channelEsName" />
          </el-select>
        </div>
        <div class="Selectli">
    ...
    <el-table :data="tableData" border stripe :max-height="maxheight" :header-cell-style="HeadTable" :row-class-name="tableRowClassName" style="width: 100%">
      ...
      <el-table-column width="280" align="center" prop="twoEstimateCost" label="V2预估费用-默认(RMB)">
        <template v-slot:header>
          <div>
            V2预估费用-默认(RMB)
            <br />
      ...
      <el-table-column width="280" align="center" prop="adjustedTwoEstimateCost" label="V2预估费用-调整后(RMB)">
        <template v-slot:header>
          <div>
            V2预估费用-调整后(RMB)
            <br />
  ...
  methods: {
    ...
    exportErrorList() {
      if (this.ImportData.length) {
        API.downExcelError({ yearAndMonth: this.filterObj.yearAndMonth, channelName: this.filterObj.channelName }).then((res) => {
          ...
          downloadFile(res, 'V2_MMC异常信息 -' + timestamp + '.xlsx') // 自定义Excel文件名
    ...
    infoByMainId(id) {
      API.infoByMainId({ mainId: id }).then((res) => {
        if (res.code === 1000) {
          if (res.data.assignee.indexOf(this.username) > -1 && res.data.version.indexOf('V2') > -1 && res.data.activityName.indexOf('审批') === -1) {
            this.isSubmit = true
          } else {
            this.isSubmit = false
          }
    ...
    downloadTemplate() {
      if (this.tableData.length) {
        ...
        API.downExcelTemplate({
          ...
          downloadFile(res, `${this.filterObj.yearAndMonth}_MMC_${this.filterObj.channelName}_V2申请.xlsx`) //自定义Excel文件名
