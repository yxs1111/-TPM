 ...
 * @LastEditTime: 2022-12-02 11:52:14
-->
<template>
  <div class="MainContent">
    ...
    <el-table :data="tableData" :max-height="maxheight" border :header-cell-style="HeadTable" :row-class-name="tableRowClassName" style="width: 100%">
      ...
      <el-table-column width="260" align="right" prop="dmItem" label="DM item">
        <template v-slot:header>
      ...
      <el-table-column width="300" align="right" prop="estimateCost" label="V2预估费用-默认(RMB)">
        <template v-slot:header>
          <div>V2预估费用-默认(RMB)<br><span class="subTitle"> KA+Brand+Dist./Vendor+DM Item</span></div>
        </template>
        <template slot-scope="scope">
          <div>
      ...
      <el-table-column width="300" align="right" prop="adjustedCost" label="V2预估费用-调整后(RMB)">
        <template v-slot:header>
          <div>V2预估费用-调整后(RMB)<br><span class="subTitle"> KA+Brand+Dist./Vendor+DM Item</span></div>
        </template>
        <template slot-scope="scope">
          <div>
    ...
    downExcel() {
      if (this.tableData.length) {
        API.exportPageExcel({
          ...
          downloadFile(
            res,
            `${this.filterObj.month}_DM_${this.filterObj.channelCode}_V2_查询.xlsx`
          ) //自定义Excel文件名
    ...
    downloadTemplate() {
      if (this.tableData.length) {
        ...
        API.exportTemplateExcel({
          ...
          downloadFile(
            res,
            `${this.filterObj.month}_DM_${this.filterObj.channelCode}_V2申请.xlsx`
          ) //自定义Excel文件名
