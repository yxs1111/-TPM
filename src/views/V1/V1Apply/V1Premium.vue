      ...
      <el-table-column width="220"
        ...
        <template v-slot:header>
          <div>
            V1计划费用(RMB)
            <br>
            <span class="subTitle"> KA + Brand + 活动类型</span>
          </div>
        </template>
        <template slot-scope="scope">
          <div>
    ...
    downExcel() {
      if (this.tableData.length) {
        API.exportExcel({
          ...
          pageNum: this.filterObj.pageNum, //posmItem
          pageSize: this.filterObj.pageSize, //posmItem
          customerCode: this.filterObj.customerCode, //客户系统名称
          channelCode: this.filterObj.channelCode, //渠道
          item: this.filterObj.item,//活动类型
          brandCode: this.filterObj.brandCode, //区域
          yearAndMonth: this.filterObj.month, //活动月
        }).then((res) => {
          downloadFile(
            res,
            `${this.filterObj.month}_Premium_${this.filterObj.channelCode}_V1_查询.xlsx`
          ) //自定义Excel文件名
